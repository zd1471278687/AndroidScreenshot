package app.zd.androidscreenshot.common.screenshot;
...
public class ScreenShotListenManager {
    private static final String TAG = ScreenShotListenManager.class.getSimpleName();
    ...
    private SocialShareDialog mShareDialog;
    ...
    private ScreenShotListenManager(Context context, Activity activity, View rootView) {
        if (context == null) {
    ...
    public static ScreenShotListenManager newInstance(Context context, Activity activity, View rootView) {
        assertInMainThread();
        return new ScreenShotListenManager(context, activity, rootView);
    }
    ...
    public void stopListen() {
        ...
        if (mInternalObserver != null) {
            try {
                ...
                Log.e(TAG, "unregisterContentObserver RuntimeException.");
            }
    ...
    private void handleMediaContentChange(Uri contentUri) {
        ...
        try {
            ...
            Log.e(TAG, "handleMediaContentChange Exception.");
        } finally {
    ...
    private void handleMediaRowData(String data, long dateTaken, int width, int height) {
        if (checkScreenShot(data, dateTaken, width, height)) {
            ...
            if (!checkCallback(data)) {
                ...
                showShareScreenShot(data); //展示截屏分享页面
            }
        } else {
    ...
    private void showShareScreenShot(String path) {
        if (mShareDialog == null) {
            mShareDialog = new SocialShareDialog(mActivity, mActivity, path);
        }
        if (mRootView != null) {
            ScreenUtil.hideSoftInput(mRootView); //隐藏软键盘
        }
        mShareDialog.show();
    }
    ...
    private static void assertInMainThread() {
        if (Looper.myLooper() != Looper.getMainLooper()) {
            StackTraceElement[] elements = Thread.currentThread().getStackTrace(); //返回一个表示该线程堆栈转储的堆栈跟踪元素数组
